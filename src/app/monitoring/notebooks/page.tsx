              ...
              <tbody>
                {notebooks.map((nb) => (
                  <tr key={nb.id}>
                    <td>{nb.name}</td>
                    <td>{nb.owner}</td>
                    <td>{nb.requests?.memory ?? "-"}</td>
                    <td>{nb.requests?.cpu ?? "-"}</td>
                    <td>{nb.requests?.["nvidia.com/gpu"] ?? "0"}</td>
                    <td>{nb.gpu?.memory ?? "0"}</td>
                    <td>{nb.hours_remaining}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <iframe src={kibanaUrl} width="1200" height="1000" />
          </>
        ) : (
          <p>
            You currently have no notebooks. You can create a notebook in{" "}
            <a className="text-decoration-none" href="/jupyterlab">
              JupyterLab
            </a>
            .
          </p>
