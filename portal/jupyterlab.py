    ...
    def clean():
        while True:
            ...
            pods = api.list_namespaced_pod(
                namespace, label_selector='k8s-app=jupyterlab').items
            for pod in pods:
...
def get_notebook(name=None, pod=None, **options):
    ...
    notebook['name'] = pod.metadata.labels.get(
        'notebook-name') or pod.metadata.labels.get('display-name')
    ...
    if expiration_date:
        time_remaining = expiration_date - \
            datetime.datetime.now(datetime.timezone.utc)
        notebook['expiration_date'] = expiration_date.isoformat()
        notebook['hours_remaining'] = int(
...
def list_notebooks():
    ...
    api = client.CoreV1Api()
    pods = api.list_namespaced_pod(
        namespace=namespace, label_selector='k8s-app=jupyterlab').items
...
def notebook_name_available(name):
    ...
    api = client.CoreV1Api()
    pods = api.list_namespaced_pod(
        namespace, label_selector='notebook-id={0}'.format(name.lower()))
