...
url = current_app.config.get('CONNECT_API_ENDPOINT')
token = current_app.config.get('CONNECT_API_TOKEN')


def get_username(globus_id):
    ''' Looks up the username for a globus ID. '''
    response = requests.get(url + '/v1alpha1/find_user',
                            params={'token': token, 'globus_id': globus_id})
    if response.text:
        data = response.json()
        if data.get('kind') == 'Error':
            current_app.logger.error(data['message'])
...
def get_usernames(group_name, **options):
    ...
    response = requests.get(url + '/v1alpha1/groups/' +
                            group_name + '/members', params={'token': token})
    if response.text:
        data = response.json()
        if data.get('kind') == 'Error':
            current_app.logger.error(data['message'])
...
def get_user_profile(username, **options):
    ...
    response = requests.get(url + '/v1alpha1/users/' +
                            username, params={'token': token})
    if response.text:
        data = response.json()
        if data.get('kind') == 'Error':
            current_app.logger.error(data['message'])
        ...
        if data.get('kind') == 'User':
            ...
            if date_format == 'object':
                ...
                profile['join_date'] = parse(
                    metadata['join_date']).strftime('%B %m %Y')
            else:
                profile['join_date'] = parse(
                    metadata['join_date']).strftime(date_format)
            ...
            membership = list(filter(
                lambda group: group['name'] == 'root.atlas-af', metadata['group_memberships']))
...
def get_user_profiles(group_name, **options):
    ...
    for username in usernames:
        request_data['/v1alpha1/users/' + username +
                     '?token=' + token] = {'method': 'GET'}
    response = requests.post(url + '/v1alpha1/multiplex',
                             params={'token': token}, json=request_data)
    if response.text:
        data = response.json()
        if data.get('kind') == 'Error':
            current_app.logger.error(data['message'])
...
def create_user_profile(**settings):
    ...
    response = requests.post(url + '/v1alpha1/users',
                             params={'token': token}, json=request_data)
    if response.text:
        data = response.json()
        if data.get('kind') == 'Error':
            current_app.logger.error(data['message'])
            raise ConnectApiError(data['message'])
    current_app.logger.info('Created profile for user %s' %
                            settings['unix_name'])
...
def update_user_profile(username, **settings):
    ...
    request_data = {'apiVersion': 'v1alpha1',
                    'kind': 'User', 'metadata': settings}
    response = requests.put(url + '/v1alpha1/users/' +
                            username, params={'token': token}, json=request_data)
    if response.text:
        data = response.json()
        if data.get('kind') == 'Error':
            current_app.logger.error(data['message'])
            raise ConnectApiError(data['message'])
    current_app.logger.info('Updated profile for user %s.' % username)
...
def get_user_groups(username, **options):
    ...
    for group_name in roles:
        request_data['/v1alpha1/groups/' + group_name +
                     '?token=' + token] = {'method': 'GET'}
    response = requests.post(url + '/v1alpha1/multiplex',
                             params={'token': token}, json=request_data)
    if response.text:
        ...
        for value in data.values():
            if value['status'] == requests.codes.ok:
                ...
                if date_format == 'object':
                    ...
                    group['creation_date'] = parse(
                        metadata['creation_date']).strftime('%B %m %Y')
                else:
                    group['creation_date'] = parse(
                        metadata['creation_date']).strftime(date_format)
...
def remove_user_from_group(username, group_name):
    ...
    response = requests.delete(url + '/v1alpha1/groups/' +
                               group_name + '/members/' + username, params={'token': token})
    if response.text:
        data = response.json()
        if data.get('kind') == 'Error':
            current_app.logger.error(data['message'])
            raise ConnectApiError(data['message'])
    current_app.logger.info('Removed user %s from group %s' %
                            (username, group_name))
...
def update_user_role(username, group_name, role):
    ...
    response = requests.put(url + '/v1alpha1/groups/' + group_name +
                            '/members/' + username, params={'token': token}, json=request_data)
    if response.text:
        data = response.json()
        if data.get('kind') == 'Error':
            current_app.logger.error(data['message'])
            raise ConnectApiError(data['message'])
    current_app.logger.info('Set role to %s for user %s in group %s' %
                            (role, username, group_name))


def get_group_info(group_name, **options):
    ''' Looks up a group and returns its info as a dictionary. '''
    response = requests.get(url + '/v1alpha1/groups/' +
                            group_name, params={'token': token})
    data = response.json()
    if data.get('kind') == 'Error':
        current_app.logger.error(data['message'])
        raise ConnectApiError(data['message'])
...
def update_group_info(group_name, **settings):
    ...
    response = requests.put(url + '/v1alpha1/groups/' +
                            group_name, params={'token': token}, json=request_data)
    if response.text:
        data = response.json()
        if data.get('kind') == 'Error':
            current_app.logger.error(data['message'])
            raise ConnectApiError(data['message'])
    current_app.logger.info('Updated info for group %s' % group_name)
    ...
    if is_group_removable(group_name):
        ...
        if response.text:
            ...
            if data.get('kind') == 'Error':
                current_app.logger.error(data['message'])
                raise ConnectApiError(data['message'])
        current_app.logger.info('Removed group %s' % group_name)
        return True
    return False
...
def get_subgroups(group_name):
    ...
    response = requests.get(url + '/v1alpha1/groups/' +
                            group_name + '/subgroups', params={'token': token})
    if response.text:
        data = response.json()
        if data.get('kind') == 'Error':
            current_app.logger.error(data['message'])
...
def create_subgroup(group_name, **settings):
    ...
    request_data = {'apiVersion': 'v1alpha1', 'metadata': settings}
    response = requests.put(url + '/v1alpha1/groups/' + group_name + '/subgroup_requests/' +
                            settings['name'], params={'token': token}, json=request_data)
    if response.text:
        data = response.json()
        if data.get('kind') == 'Error':
            current_app.logger.error(data['message'])
            raise ConnectApiError(data['message'])
    current_app.logger.info('Created subgroup %s in group %s' %
